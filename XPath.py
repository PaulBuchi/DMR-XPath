...
def build_edge_model(venues):
    ...
    for venue, years in venues.items():
        ...
        for year, pubs in years.items():
            ...
            for pub in pubs:
                pub_node = Node(pub.tag, s_id=pub.get("key"))
                for child in pub:
...
def test_queries(cur):
    ...
    result = cur.fetchone()
    if result:
        print(ancestor_nodes(cur, result[0]))

    print("Descendants von VLDB 2023 (id=2):")
    print(descendant_nodes(cur, 2))

    print("Siblings von 'SchmittKAMM23':")
    cur.execute("SELECT id FROM Node WHERE s_id = 'SchmittKAMM23'")
    sid_result = cur.fetchone()
    if sid_result:
        print("Following:")
        print(siblings(cur, sid_result[0], "following"))
        print("Preceding:")
        print(siblings(cur, sid_result[0], "preceding"))
